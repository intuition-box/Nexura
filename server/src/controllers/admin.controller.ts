...
export const getAdminLeaderboard = async (req: GlobalRequest, res: GlobalResponse) => {
	try {
		...
		const items = await user
			...
			.select("_id address username profilePic eventsWon createdAt lessonsCompleted xp level questsCompleted campaignsCompleted noOfMints")
			.lean();
...
export const createEcosystemDapp = async (req: GlobalRequest, res: GlobalResponse) => {
  try {
    const { name, description, category, websiteUrl, reward } = req.body;
    const logoFile = req.file;

    if (!name || !websiteUrl) {
      res.status(BAD_REQUEST).json({ error: "Name and website URL are required." });
      return;
    }

    // Logo can come from file upload OR from a URL string (backward compat)
    let logoValue = "";
    if (logoFile) {
      logoValue = await uploadImg({
        file: logoFile.buffer,
        filename: logoFile.originalname || "logo.png",
        folder: "ecosystem-dapps",
      });
    } else if (req.body.logo && typeof req.body.logo === "string" && req.body.logo.trim()) {
      logoValue = String(req.body.logo).trim();
    } else {
      res.status(BAD_REQUEST).json({ error: "Logo image or logo URL is required." });
      return;
    }
    ...
    res.status(OK).json({ message: "Ecosystem dapp created.", logo: logoValue });
  } catch (error: any) {
    logger.error(error?.message || error);
    console.error("[createEcosystemDapp]", error?.message || error);
    if (error?.code === 11000) {
      res.status(BAD_REQUEST).json({ error: "A dapp with this name already exists." });
      return;
    }
    res.status(INTERNAL_SERVER_ERROR).json({ error: "Error creating ecosystem dapp" });
  }
};
...
export const getEcosystemDapps = async (_req: GlobalRequest, res: GlobalResponse) => {
  try {
    ...
    logger.error(error);
    res.status(INTERNAL_SERVER_ERROR).json({ error: "Error fetching ecosystem dapps" });
  }
};

export const updateEcosystemDapp = async (req: GlobalRequest, res: GlobalResponse) => {
  try {
    ...
    const existing = await ecosystemQuest.findById(id);
    if (!existing) {
      res.status(NOT_FOUND).json({ error: "Dapp not found." });
      return;
    }

    const { name, description, category, websiteUrl, reward, logo: logoBody } = req.body;
    const logoFile = req.file;
    ...
    if (websiteUrl !== undefined) existing.websiteUrl = String(websiteUrl).trim();
    if (reward !== undefined) existing.reward = Number(reward) || 0;

    // Update logo if provided (file takes priority, then URL string)
    if (logoFile) {
      existing.logo = await uploadImg({
        file: logoFile.buffer,
        filename: logoFile.originalname || "logo.png",
        folder: "ecosystem-dapps",
      });
    } else if (logoBody && typeof logoBody === "string" && logoBody.trim()) {
      existing.logo = String(logoBody).trim();
    }

    await existing.save();
    res.status(OK).json({ message: "Ecosystem dapp updated.", logo: existing.logo });
  } catch (error: any) {
    logger.error(error?.message || error);
    console.error("[updateEcosystemDapp]", error?.message || error);
    if (error?.code === 11000) {
      res.status(BAD_REQUEST).json({ error: "A dapp with this name already exists." });
      return;
    }
    res.status(INTERNAL_SERVER_ERROR).json({ error: "Error updating ecosystem dapp" });
  }
};

export const deleteEcosystemDapp = async (req: GlobalRequest, res: GlobalResponse) => {
  try {
    const { id } = req.query as unknown as { id: string };
    if (!id) {
      res.status(BAD_REQUEST).json({ error: "Dapp id is required." });
    ...
    logger.error(error);
    res.status(INTERNAL_SERVER_ERROR).json({ error: "Error deleting ecosystem dapp" });
  }
};
